...
function _nnmtf_proxgrad(
    ...
    while i < maxiter
        ...
        proj!(A; projection=projectionA, metric=metricA, dims=to_dims(normalizeA)) # Want the rows of A normalized when using :simplex projection
        ...
        proj!(B; projection=projectionB, metric=metricB, dims=to_dims(normalize))

        rescale_AB ? rescaleAB!(A, B; normalize, metricA, metricB, scaleBtoA) : nothing
...
function converged(; dist_Ncone, i, A, B, A_last, B_last, tol, problem_size, criterion, Y, Yhat, normalize)
    ...
    if !(criterion in IMPLIMENTED_CRITERIA)
        return throw(UnimplimentedError("criterion is not an impliment criterion"))
...
function to_dims(normalize::Symbol)
    if normalize == :fibres
        return (1, 2)
    elseif normalize == :slices || normalize == :rows
        return 1
    ...
    elseif normalize == :nothing
        ...
        return throw(UnimplimentedError("normalize is not an implimented normalization"))
    end
end
