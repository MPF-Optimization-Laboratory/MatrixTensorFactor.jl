...
GL(x) = x'*laplacian_matrix(length(x))*x
∇GL(x) = laplacian_matrix(length(x))*x
...
L(x; Δt=Δt, λ=λ, A=A, y=y) = 0.5 * norm(A*x - y)^2 + λ*GL(x)
∇L(x; Δt=Δt, λ=λ, A=A, y=y) = A'*(A*x - y) + λ*∇GL(x)
...
function solve_problem(A, y; x_init=initialize_x(size(A, 2)), grad_tol=0.0001, rel_tol=0.002, max_itr=1500, Δt=Δt)
    ...
    x = x_init
    #α = 1 / sqrt(opnorm(Symmetric(A*A'))^2 + λ^2*norm(∇TV(x; Δt))^2)
    α = 1 / opnorm(Symmetric(A'A)+λ*Symmetric(laplacian_matrix(n)))
    i = 1
