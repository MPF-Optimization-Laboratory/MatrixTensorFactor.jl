...
$$
\min_{A_1, \dots, A_N} \left\lVert g(A_1,\dots,A_N) - Y\right\rVert _F^2 \quad \text{s.t.} \quad (A_1,\dots,A_N) \in\mathcal{C}_1 \times \dots \times \mathcal{C}_N
...
function _factorize(Y; kwargs...)
    ...
    parameters, updateparameters! =
        initialize_parameters(decomposition, Y, previous; kwargs...)

    # more kwargs get added here, so need to return kwargs
    update!, kwargs = make_update!(decomposition, Y; kwargs...)

    stats_data, getstats =
        initialize_stats(decomposition, Y, previous, parameters; kwargs...)
    ...
    while !converged(stats_data; kwargs...)
        ...
        # This is one cycle of updates on each factor in the decomposition
        ...
        # Update the one or two previous iterates
...
Abstracting further, we can see the general structure. -->
