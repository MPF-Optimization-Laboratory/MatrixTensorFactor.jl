...
For example, the demixing of $R$ probibility densities $b_1, \dots, b_R$ from $I$ mixtures $y_1,\dots,y_I$ for $I>R$ can be accomplished with a nonneagive matrix factorization (cite). We have the system of equations
...
The Eucledian simplex projection can be done with the following implimentation of Chen and Ye's algorithm [@chen_projection_2011]. The essence of the algorithm is to efficiently compute the special $t\in \mathbb{R}$ so that
...
and then apply the divergence projection.^[In the unfortunate case where every entry of $v$ is nonpositive, we can fallback to the Euclean simplex projection.] All together, this looks like
$$
